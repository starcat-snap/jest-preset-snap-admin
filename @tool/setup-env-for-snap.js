...
if (!srcPath) {
  throw new Error(
    '"globals.adminPath" is not defined. A file path to a SnapAdmin administration is required'
  );
}
...
if (vue3) {
  ...
  const configureCompat = require(resolve(
    join(srcPath, "node_modules/@vue/compat/index.js")
  )).configureCompat;
  configureCompat({
...
const SnapAdmin = require(resolve(
  join(srcPath, `src/core/snap-admin.ts`)
)).default;

module.exports = (() => {
  global.SnapAdmin = SnapAdmin;
  ...
  require(resolve(
    join(srcPath, "src/app/init/component-helper.init")
